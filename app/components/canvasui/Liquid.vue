...
export function createLiquid(
  ...
  if (htmlInCanvas) {
    let paintRetryScheduled = false;
    paintable.onpaint = () => {
      try {
        sourceCtx!.reset();
        sourceCtx!.drawElementImage!(content, 0, 0);
        contentDirty = true;
        paintRetryScheduled = false;
        wake();
      } catch (err) {
        // L'API HTML-in-Canvas est experimentale : juste apres un resize du
        // canvas source, drawElementImage peut echouer le temps qu'un tick
        // de layout se termine. On reessaie au lieu d'abandonner en silence,
        // sinon l'effet reste vide en permanence des le premier echec.
        if (!paintRetryScheduled) {
          paintRetryScheduled = true;
        ...
        if (typeof console !== "undefined" && console.debug) {
          console.debug("[canvasui] paint retry after error:", err);
        }
      }
    };
  }
  ...
  function syncCanvasSize() {
    ...
    if (htmlInCanvas) {
      ...
      if (source.width !== cssWidth * dpr || source.height !== cssHeight * dpr) {
        ...
        // Changer width/height reinitialise le canvas et son layout interne.
        // On laisse un tick de layout se stabiliser avant de redemander une
        // peinture, sinon le premier drawElementImage post-resize echoue
        // souvent avec cette API encore experimentale.
...
const props = defineProps<LiquidOptions>();
